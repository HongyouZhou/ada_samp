...
from core.utils.misc import link_untrained_params
from models.denoising.registry import get_model, register_model



@register_model("Diffusion2D")
class Diffusion2D(nn.Module):
    ...
    def __init__(self,
        ...
        self.diffusion = get_model(diffusion)
        self.diffusion_use_ema = diffusion_use_ema
        if self.diffusion_use_ema:
            if inference_only:
                self.diffusion_ema = self.diffusion
            else:
                self.diffusion_ema = get_model(diffusion)  # deepcopy doesn't work due to the monkey patch lora
