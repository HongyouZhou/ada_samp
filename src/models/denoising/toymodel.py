...
def get_1d_sincos_pos_embed(embed_dim, pos, min_period=1e-3, max_period=10):
    if embed_dim % 2 != 0:
        raise ValueError("embed_dim must be divisible by 2")
    half_dim = embed_dim // 2
    period = torch.logspace(math.log(min_period), math.log(max_period), half_dim, base=math.e, dtype=pos.dtype, device=pos.device)
    out = pos.unsqueeze(-1) * (2 * math.pi / period)
    emb = torch.cat([torch.sin(out), torch.cos(out)], dim=-1)
    return emb


def get_2d_sincos_pos_embed(embed_dim, crd, min_period=1e-3, max_period=10):
    """
    Args:
        embed_dim (int)
    ...
    if embed_dim % 2 != 0:
        raise ValueError("embed_dim must be divisible by 2")
    bs = crd.size(0)
    emb = get_1d_sincos_pos_embed(embed_dim // 2, crd.flatten(), min_period, max_period)  # (bs * 2, embed_dim // 2)
