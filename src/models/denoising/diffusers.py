...
from diffusers.models import AutoencoderKL, ControlNetModel, DiTTransformer2DModel
from diffusers.models import UNet2DConditionModel as _UNet2DConditionModel
from diffusers.models.attention_processor import Attention, AttnProcessor, AttnProcessor2_0, LoRAAttnProcessor, LoRAAttnProcessor2_0, LoRAXFormersAttnProcessor, XFormersAttnProcessor
...
from .utils import constant_init, get_root_logger, kaiming_init, load_checkpoint, rgetattr
...
def build_module(cfg):
    if isinstance(cfg, dict) and "type" in cfg:
        cls = LORA_PROCESSOR_MAP[cfg["type"]]
        cfg.pop("type")
        return cls(**cfg)
    else:
        raise ValueError("Unsupported config for build_module: {}".format(cfg))
...
def unet_enc(
    ...
    encoder_hidden_states = unet.process_encoder_hidden_states(encoder_hidden_states=encoder_hidden_states, added_cond_kwargs=added_cond_kwargs)
...
class UNet2DConditionModel(_UNet2DConditionModel):
    def __init__(self, *args, freeze=True, freeze_exclude=[], pretrained=None, torch_dtype="float32", freeze_exclude_fp32=True, **kwargs):
        ...
        self.set_use_memory_efficient_attention_xformers(not hasattr(torch.nn.functional, "scaled_dot_product_attention"))
    ...
    def init_weights(self, pretrained):
        if pretrained is not None:
            ...
            checkpoint = load_checkpoint(pretrained, map_location="cpu", logger=logger)
            ...
            if state_dict["conv_in.weight"].size() != self.conv_in.weight.size():
                ...
                state_dict["conv_out.weight"] = torch.einsum("ts,scxy->tcxy", convert_mat_out, state_dict["conv_out.weight"])
                state_dict["conv_out.bias"] = torch.einsum("ts,s->t", convert_mat_out, state_dict["conv_out.bias"])
                state_dict["conv_in.weight"] = torch.einsum("st,csxy->ctxy", convert_mat_in, state_dict["conv_in.weight"])
            self.load_state_dict(state_dict, strict=False)

    def forward(self, sample, timestep, encoder_hidden_states, **kwargs):
        dtype = sample.dtype
        return super().forward(sample, timestep, encoder_hidden_states, return_dict=False, **kwargs)[0].to(dtype)
...
class UNetLoRAWrapper(nn.Module):
    def __init__(self, unet, lora_layers=dict(type="LoRAXFormersAttnProcessor", rank=4)):
        super().__init__()
        self.unet: UNet2DConditionModel = UNet2DConditionModel(**unet)
        lora_attn_procs = {}
        for name in self.unet.attn_processors:
            ...
            lora_cfg.update(dict(hidden_size=hidden_size, cross_attention_dim=cross_attention_dim))
...
class CLIPLoRAWrapper(nn.Module):
    def __init__(self, text_encoder, lora_layers=dict(type="LoRAXFormersAttnProcessor", rank=4)):
        super().__init__()
        ...
        for name, module in self.text_encoder.named_modules():
            if name.endswith(TEXT_ENCODER_ATTN_MODULE):
                lora_cfg = deepcopy(lora_layers)
                lora_cfg.update(dict(hidden_size=module.out_proj.out_features, cross_attention_dim=None))
                m = build_module(lora_cfg)
...
class VAEDecoder(Decoder, ModelMixin):
    def __init__(self, in_channels=12, out_channels=24, up_block_types=("UpDecoderBlock2D",), block_out_channels=(64,), layers_per_block=2, norm_num_groups=32, act_fn="silu", norm_type="group", zero_init_residual=True):
        super(VAEDecoder, self).__init__(
            in_channels=in_channels, out_channels=out_channels, up_block_types=up_block_types, block_out_channels=block_out_channels, layers_per_block=layers_per_block, norm_num_groups=norm_num_groups, act_fn=act_fn, norm_type=norm_type
        )
        self.set_use_memory_efficient_attention_xformers(not hasattr(torch.nn.functional, "scaled_dot_product_attention"))
...
class PretrainedUNet(nn.Module):
    def __init__(self, from_pretrained=None, freeze=True, torch_dtype="float32", checkpointing=True, use_ref_attn=False, **kwargs):
        super().__init__()
        if torch_dtype is not None:
            kwargs.update(torch_dtype=getattr(torch, torch_dtype))
        self.unet = _UNet2DConditionModel.from_pretrained(from_pretrained, **kwargs)
        self.freeze = freeze
        if self.freeze:
            self.requires_grad_(False)
        self.unet.set_use_memory_efficient_attention_xformers(not hasattr(torch.nn.functional, "scaled_dot_product_attention"))
        if use_ref_attn:
            unet_attn_procs = dict()
            for name, attn_proc in self.unet.attn_processors.items():
                unet_attn_procs[name] = ReferenceAttnProc(attn_proc, enabled=name.endswith("attn1.processor"), name=name)
            self.unet.set_attn_processor(unet_attn_procs)
        if checkpointing:
            self.unet.enable_gradient_checkpointing()

    def forward(self, sample, timestep, encoder_hidden_states, **kwargs):
        return self.unet(sample, timestep, encoder_hidden_states, return_dict=False, **kwargs)[0]
...
class PretrainedControlNet(nn.Module):
    def __init__(self, from_pretrained=None, freeze=True, torch_dtype="float32", checkpointing=True, **kwargs):
        super().__init__()
        if torch_dtype is not None:
            kwargs.update(torch_dtype=getattr(torch, torch_dtype))
        self.controlnet = ControlNetModel.from_pretrained(from_pretrained, **kwargs)
        self.freeze = freeze
        if self.freeze:
            self.requires_grad_(False)
        self.controlnet.set_use_memory_efficient_attention_xformers(not hasattr(torch.nn.functional, "scaled_dot_product_attention"))
        if checkpointing:
...
class PretrainedCLIPTextModelWithProjection(nn.Module):
    def __init__(self, from_pretrained=None, freeze=True, torch_dtype="float32", **kwargs):
        super().__init__()
        if torch_dtype is not None:
            kwargs.update(torch_dtype=getattr(torch, torch_dtype))
        self.model = CLIPTextModelWithProjection.from_pretrained(from_pretrained, **kwargs)
        self.freeze = freeze
        if self.freeze:
            self.requires_grad_(False)
...
class PretrainedT5EncoderModel(nn.Module):
    def __init__(self, from_pretrained=None, freeze=True, torch_dtype="float32", **kwargs):
        ...
        if torch_dtype is not None:
            kwargs.update(torch_dtype=getattr(torch, torch_dtype))
        self.model = T5EncoderModel.from_pretrained(from_pretrained, **kwargs)
        self.freeze = freeze
        if self.freeze:
            self.requires_grad_(False)
...
class PretrainedDiT(nn.Module):
    def __init__(self, from_pretrained=None, pos_embed_path=None, freeze=True, torch_dtype="float32", checkpointing=True, **kwargs):
        super().__init__()
        if torch_dtype is not None:
            kwargs.update(torch_dtype=getattr(torch, torch_dtype))
        self.dit = DiTTransformer2DModel.from_pretrained(from_pretrained, **kwargs)
    ...
    def forward(
        ...
        return self.dit.forward(hidden_states=hidden_states, timestep=timestep, class_labels=class_labels, cross_attention_kwargs=cross_attention_kwargs, return_dict=False)[0]
...
class PretrainedVAE(nn.Module):
    def __init__(self, from_pretrained=None, del_encoder=False, del_decoder=False, use_slicing=False, freeze=True, torch_dtype="float32", **kwargs):
        super().__init__()
        if torch_dtype is not None:
            kwargs.update(torch_dtype=getattr(torch, torch_dtype))
        self.vae = AutoencoderKL.from_pretrained(from_pretrained, **kwargs)
        if del_encoder:
        ...
        if self.freeze:
            self.requires_grad_(False)
        self.vae.set_use_memory_efficient_attention_xformers(not hasattr(torch.nn.functional, "scaled_dot_product_attention"))
    ...
    def visualize(self, code, scene_name, viz_dir):
        code_viz = (self(code).to(dtype=torch.float32, device="cpu").numpy().transpose(0, 2, 3, 1) / 2 + 0.5).clip(min=0, max=1)
...
class PretrainedVAEDecoder(PretrainedVAE):
    def __init__(self, **kwargs):
        super().__init__(del_encoder=True, del_decoder=False, **kwargs)
...
class PretrainedVAEEncoder(PretrainedVAE):
    def __init__(self, **kwargs):
        super().__init__(del_encoder=False, del_decoder=True, **kwargs)
...
class ReferenceAttnProc(torch.nn.Module):
    ...
    def __call__(self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None, mode="w", ref_dict: dict = None):
