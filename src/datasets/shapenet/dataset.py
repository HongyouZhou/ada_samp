...
shapenetpart_cat2id = {"airplane": 0, "bag": 1, "cap": 2, "car": 3, "chair": 4, "earphone": 5, "guitar": 6, "knife": 7, "lamp": 8, "laptop": 9, "motor": 10, "mug": 11, "pistol": 12, "rocket": 13, "skateboard": 14, "table": 15}
...
class ShapeNetPart(data.Dataset):
    def __init__(
        ...
        load_file=True,
        segmentation=False,
        segmentation_one_hot=False,
        random_rotate=False,
        random_jitter=False,
        random_translate=False,
    ):
        assert dataset_name.lower() in ["shapenetcorev2", "shapenetpart", "modelnet10", "modelnet40", "shapenetpartpart"]
        ...
        if dataset_name not in ["shapenetpart"] and segmentation is True:
        ...
        self.segmentation_one_hot = segmentation_one_hot
        ...
        if self.split in ["train", "trainval", "all"]:
            self.get_path("train")
        if self.dataset_name in ["shapenetcorev2", "shapenetpart", "shapenetpartpart"] and self.split in ["val", "trainval", "all"]:
            self.get_path("val")
    ...
    def get_path(self, type):
        path_h5py = os.path.join(self.root, f"{type}*.h5")
...
if __name__ == "__main__":
    ...
    d = ShapeNetPart(root=root, dataset_name=dataset_name, num_points=2048, split=split, segmentation=True)
    print("datasize:", d.__len__())

    item = 0
    ps, lb, seg, n, f = d[item]
    print(ps.size(), ps.type(), lb.size(), lb.type(), seg.size(), seg.type(), n, f)
