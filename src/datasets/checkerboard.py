...
@register_dataset("CheckerboardData")
class CheckerboardData(Dataset):
    def __init__(
        ...
        white_squares = [
            (i, j) for i in range(n_rc) for j in range(n_rc) if (i + j) % 2 == 0
        ]
    ...
    def draw_samples(self, n_samples):
        ...
        if self.thickness < 1:
            ...
            square_samples_angle = torch.atan2(
                square_samples[:, 1], square_samples[:, 0]
            ).unsqueeze(-1)
            max_r = torch.minimum(
                0.5 / square_samples_angle.cos().abs().clamp(min=1e-6),
                0.5 / square_samples_angle.sin().abs().clamp(min=1e-6),
            ).square()
            square_samples_r_scaled = (
                max_r - (max_r - square_samples_r) * self.thickness**0.5
            )
