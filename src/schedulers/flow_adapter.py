...
class FlowAdapterScheduler(SchedulerMixin, ConfigMixin):
    ...
    @register_to_config
    def __init__(self, num_train_timesteps: int = 1000, shift: float = 1.0, base_scheduler="DPMSolverMultistep", eps=1e-6, **kwargs):
        ...
        if base_scheduler in ["EulerDiscrete", "EulerAncestralDiscrete"]:
            self.scales = ((alphas**2 + self.sigmas**2) / (1 + (self.sigmas / alphas.clamp(min=self.config.eps)) ** 2)).sqrt()
        elif base_scheduler in ["DPMSolverSinglestep", "DPMSolverMultistep", "DEISMultistep", "SASolver", "UniPCMultistep"]:
        ...
        self.base_scheduler = base_scheduler_class(num_train_timesteps=num_train_timesteps, prediction_type="epsilon", **kwargs)
    ...
    def set_timesteps(self, num_inference_steps: int, device=None):
        ...
        sigmas = torch.from_numpy(1 - np.linspace(0, 1, num_inference_steps, dtype=np.float32, endpoint=False))
        ...
        if self.config.base_scheduler in ["EulerDiscrete", "EulerAncestralDiscrete"]:
            ...
            self.base_scheduler.sigmas = self.sigmas / alphas.clamp(min=self.config.eps)
            self.scales = ((alphas**2 + self.sigmas**2) / (1 + (self.sigmas / alphas.clamp(min=self.config.eps)) ** 2)).sqrt()
        elif self.config.base_scheduler in ["DPMSolverSinglestep", "DPMSolverMultistep", "DEISMultistep", "SASolver", "UniPCMultistep"]:
            self.base_scheduler.timesteps = self.timesteps
            self.base_scheduler.sigmas = self.sigmas / alphas.clamp(min=self.config.eps)
            self.scales = (alphas**2 + self.sigmas**2).sqrt()
        else:
    ...
    def step(
        self, model_output: torch.FloatTensor, timestep: float | torch.FloatTensor, sample: torch.FloatTensor, generator: torch.Generator | None = None, return_dict: bool = True, prediction_type="u", eps=1e-6
    ) -> FlowWrapperSchedulerOutput | tuple:
        ...
        prev_sample = self.base_scheduler.step(epsilon, timestep, sample / scale, **kwargs)[0] * next_scale
